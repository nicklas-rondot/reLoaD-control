...
@login_required
def applications():
    applications = Application.query.filter_by(user_model_id=current_user.id)
    context = {
...
def application():
    ...
    if id_str == "new":
        application = Application(
            name=f"New Application", user_model_id=current_user.id
        )
    ...
    modules = Module.query.filter_by(user_model_id=current_user.id)
    context = {
        "application": application,
        "modules": modules,
        "current_user": current_user,
...
def delete_application():
    ...
    application = Application.query.filter_by(
        name=application_name, user_model_id=current_user.id
    ).first()
...
@app.route("/add_multiple_functions")
@login_required
def add_multiple_functions():
    amount = request.args.get("amount")
    module_str = request.args.get("module")
    module = Module.query.filter_by(name=module_str).first()
    print(module_str)

    context = {"amount": amount, "module": module}

    return render_template("add_action_multiple_functions.html", context=context)
...
def add_function_variables():
    ...
    context = {"function": function, "i": i}

    return render_template("add_action_function_variables.html", context=context)
...
def save_module():
    ...
    module = Module.query.filter_by(
        name=module_name, user_model_id=current_user.id
    ).first()

    old_function_names = [
        old_function.name for old_function in Function.query.filter_by(module=module)
    ]
    new_function_names = []
    ...
    while True:
        ...
        for variable_name in variables:
            ...
            variable = Variable.query.filter_by(
                name=variable_name, function=function
            ).first()
            if variable == None:
                variable = Variable(name=variable_name, function=function)
            db.session.add(variable)
